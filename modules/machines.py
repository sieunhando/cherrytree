...
class XMLHandler:
    ...
    def dom_to_treestore(self, ctd, discard_ids, tree_father=None):
        ...
        if cherrytree.nodeName != cons.APP_NAME:
            return False
        else:
    ...
    def append_tree_node(self, dom_iter, tree_father, discard_ids, node_sequence):
        ...
        custom_icon_id = int(dom_iter.attributes['custom_icon_id'].value) if dom_iter.hasAttribute(
            'custom_icon_id') else 0
        ...
        while child_dom_iter != None:
            ...
            elif child_dom_iter.nodeName == "encoded_png":
                self.image_deserialize(curr_buffer, child_dom_iter, 2)
            elif child_dom_iter.nodeName == "table":
                self.table_deserialize(curr_buffer, child_dom_iter)
            elif child_dom_iter.nodeName == "codebox":
                self.codebox_deserialize(curr_buffer, child_dom_iter)
            elif child_dom_iter.nodeName == "encoded_image":
                self.image_deserialize(curr_buffer, child_dom_iter, 1)
            elif child_dom_iter.nodeName == "node":
                break
            child_dom_iter = child_dom_iter.nextSibling
    ...
    def codebox_deserialize(self, curr_buffer, dom_node):
        ...
        justification = dom_node.attributes[cons.TAG_JUSTIFICATION].value if dom_node.hasAttribute(
            cons.TAG_JUSTIFICATION) else cons.TAG_PROP_LEFT
        codebox_dict = {
            'frame_width': int(dom_node.attributes['frame_width'].value),
            'frame_height': int(dom_node.attributes['frame_height'].value),
            'width_in_pixels': dom_node.hasAttribute("width_in_pixels") and dom_node.attributes[
                                                                                'width_in_pixels'].value == "True",
            'syntax_highlighting': dom_node.attributes['syntax_highlighting'].value,
            'highlight_brackets': dom_node.hasAttribute("highlight_brackets") and dom_node.attributes[
                                                                                      'highlight_brackets'].value == "True",
            'show_line_numbers': dom_node.hasAttribute("show_line_numbers") and dom_node.attributes[
    ...
    def table_deserialize(self, curr_buffer, dom_node):
        ...
        char_offset = int(dom_node.attributes["char_offset"].value)
        if dom_node.hasAttribute(cons.TAG_JUSTIFICATION):
            justification = dom_node.attributes[cons.TAG_JUSTIFICATION].value
        else:
    ...
    def image_deserialize(self, curr_buffer, dom_node, version):
        ...
        if dom_node.hasAttribute("anchor"):
            ...
            if version == 2:
                pixbuf = get_pixbuf_from_encoded_buffer(dom_node.firstChild.data)
            else:
                pixbuf = get_pixbuf_from_png_encoded_string(dom_node.firstChild.data)
        if pixbuf:
    ...
    def rich_text_deserialize(self, curr_buffer, dom_node):
        ...
        if tags_num == 0:
            curr_buffer.insert(curr_buffer.get_end_iter(), text)
        else:
            curr_buffer.insert_with_tags_by_name(curr_buffer.get_end_iter(), text, *tag_names)
            ...
            if to_disk == True:
                ...
                for element in pixbuf_table_codebox_vector:
                    ...
                    if element[0] == "pixbuf":
                        ...
                        self.table_element_to_xml(element[1], dom_iter, self.dom)
                    elif element[0] == "codebox":
                        self.codebox_element_to_xml(element[1], dom_iter, self.dom)
        else:
            # plain text insert
    ...
    def rich_txt_serialize(self, dom_node, start_iter, end_iter, curr_attributes, change_case="n", dom=None):
        ...
        if change_case != "n":
            if change_case == "l":
                slot_text = slot_text.lower()
            elif change_case == "u":
                slot_text = slot_text.upper()
            elif change_case == "t":
                slot_text = slot_text.swapcase()
    ...
    def rich_text_attributes_update(self, curr_iter, curr_attributes):
        ...
        for tag in toggled_off:
            ...
            if tag_name and tag_name != cons.GTKSPELLCHECK_TAG_NAME:
                if tag_name.startswith("weight_"):
                    curr_attributes[cons.TAG_WEIGHT] = ""
                elif tag_name.startswith("foreground_"):
                    curr_attributes[cons.TAG_FOREGROUND] = ""
                elif tag_name.startswith("background_"):
                    curr_attributes[cons.TAG_BACKGROUND] = ""
                elif tag_name.startswith("style_"):
                    curr_attributes[cons.TAG_STYLE] = ""
                elif tag_name.startswith("underline_"):
                    curr_attributes[cons.TAG_UNDERLINE] = ""
                elif tag_name.startswith("strikethrough_"):
                    curr_attributes[cons.TAG_STRIKETHROUGH] = ""
                elif tag_name.startswith("scale_"):
                    curr_attributes[cons.TAG_SCALE] = ""
                elif tag_name.startswith("justification_"):
                    curr_attributes[cons.TAG_JUSTIFICATION] = ""
                elif tag_name.startswith("link_"):
                    curr_attributes[cons.TAG_LINK] = ""
                elif tag_name.startswith("family_"):
                    curr_attributes[cons.TAG_FAMILY] = ""
                else:
                    support.dialog_error("Failure processing the toggling OFF tag %s" % tag_name, self.dad.window)
        toggled_on = curr_iter.get_toggled_tags(toggled_on=True)
        for tag in toggled_on:
            tag_name = tag.get_property("name")
            if tag_name and tag_name != cons.GTKSPELLCHECK_TAG_NAME:
                if tag_name.startswith("weight_"):
                    curr_attributes[cons.TAG_WEIGHT] = tag_name[7:]
                elif tag_name.startswith("foreground_"):
                    curr_attributes[cons.TAG_FOREGROUND] = tag_name[11:]
                elif tag_name.startswith("background_"):
                    curr_attributes[cons.TAG_BACKGROUND] = tag_name[11:]
                elif tag_name.startswith("scale_"):
                    curr_attributes[cons.TAG_SCALE] = tag_name[6:]
                elif tag_name.startswith("justification_"):
                    curr_attributes[cons.TAG_JUSTIFICATION] = tag_name[14:]
                elif tag_name.startswith("style_"):
                    curr_attributes[cons.TAG_STYLE] = tag_name[6:]
                elif tag_name.startswith("underline_"):
                    curr_attributes[cons.TAG_UNDERLINE] = tag_name[10:]
                elif tag_name.startswith("strikethrough_"):
                    curr_attributes[cons.TAG_STRIKETHROUGH] = tag_name[14:]
                elif tag_name.startswith("link_"):
                    curr_attributes[cons.TAG_LINK] = tag_name[5:]
                elif tag_name.startswith("family_"):
                    curr_attributes[cons.TAG_FAMILY] = tag_name[7:]
                else:
                    support.dialog_error("Failure processing the toggling ON tag %s" % tag_name, self.dad.window)
    ...
    def toc_insert_all(self, text_buffer, top_tree_iter):
        ...
        if top_tree_iter:
            toc_list_per_node = self.toc_insert_all_iter(top_tree_iter)
        else:
        ...
        if toc_list_per_node:
            ...
            for element in toc_list_per_node:
                ...
                if curr_node_id != element[2]:
                    ...
                    node_name = self.dad.treestore[node_tree_iter][1].replace(cons.CHAR_NEWLINE,
                                                                              cons.CHAR_SPACE).replace(cons.CHAR_CR, "")
                    tag_names = [self.dad.apply_tag_exist_or_create(tag_property, property_value)]
                    text_buffer.insert(text_buffer.get_iter_at_offset(curr_offset),
                                       2 * curr_node_level * cons.CHAR_SPACE + self.dad.chars_toc[0] + cons.CHAR_SPACE)
                    curr_offset += 2 + 2 * curr_node_level
                    text_buffer.insert(text_buffer.get_iter_at_offset(curr_offset), cons.CHAR_NEWLINE)
                    text_buffer.insert_with_tags_by_name(text_buffer.get_iter_at_offset(curr_offset), node_name,
                                                         *tag_names)
                    curr_offset += 1
                    ...
                    curr_offset += 1
                tag_names = [
                    self.dad.apply_tag_exist_or_create(tag_property, property_value + cons.CHAR_SPACE + element[0])]
                if not element[0]: continue
                if element[0][:2] == cons.TAG_PROP_H1:
                    text_buffer.insert(text_buffer.get_iter_at_offset(curr_offset),
                                       (2 + 2 * curr_node_level) * cons.CHAR_SPACE + self.dad.chars_toc[
                                           1] + cons.CHAR_SPACE)
                    curr_offset += 4 + 2 * curr_node_level
                elif element[0][:2] == cons.TAG_PROP_H2:
                    text_buffer.insert(text_buffer.get_iter_at_offset(curr_offset),
                                       (4 + 2 * curr_node_level) * cons.CHAR_SPACE + self.dad.chars_toc[
                                           2] + cons.CHAR_SPACE)
                    curr_offset += 6 + 2 * curr_node_level
                else:
                    text_buffer.insert(text_buffer.get_iter_at_offset(curr_offset),
                                       (6 + 2 * curr_node_level) * cons.CHAR_SPACE + self.dad.chars_toc[
                                           3] + cons.CHAR_SPACE)
                    curr_offset += 8 + 2 * curr_node_level
                text_buffer.insert(text_buffer.get_iter_at_offset(curr_offset), cons.CHAR_NEWLINE)
                text_buffer.insert_with_tags_by_name(text_buffer.get_iter_at_offset(curr_offset), element[1],
                                                     *tag_names)
                curr_offset += 1
                while text_buffer.get_iter_at_offset(curr_offset).get_char() != cons.CHAR_NEWLINE:
                    curr_offset += 1
    ...
    def toc_insert_one(self, text_buffer, node_id, just_get_toc_list=False):
        ...
        while tag_found:
            ...
            self.toc_insert_parser(text_buffer, start_iter, curr_iter, node_id)
        if self.toc_list and not just_get_toc_list:
            ...
            for element in self.toc_list:
                ...
                if element[0][:2] == cons.TAG_PROP_H1:
                    text_buffer.insert(text_buffer.get_iter_at_offset(curr_offset),
                                       self.dad.chars_toc[1] + cons.CHAR_SPACE)
                    curr_offset += 2
                elif element[0][:2] == cons.TAG_PROP_H2:
                    text_buffer.insert(text_buffer.get_iter_at_offset(curr_offset),
                                       2 * cons.CHAR_SPACE + self.dad.chars_toc[2] + cons.CHAR_SPACE)
                    curr_offset += 4
                else:
                    text_buffer.insert(text_buffer.get_iter_at_offset(curr_offset),
                                       4 * cons.CHAR_SPACE + self.dad.chars_toc[3] + cons.CHAR_SPACE)
                    curr_offset += 6
                text_buffer.insert(text_buffer.get_iter_at_offset(curr_offset), cons.CHAR_NEWLINE)
                text_buffer.insert_with_tags_by_name(text_buffer.get_iter_at_offset(curr_offset), element[1],
                                                     *tag_names)
    ...
    def toc_insert_parser(self, text_buffer, start_iter, end_iter, node_id):
        ...
        if self.curr_attributes[cons.TAG_SCALE] not in [cons.TAG_PROP_H1, cons.TAG_PROP_H2,
                                                        cons.TAG_PROP_H3]: return None
        ...
        if self.curr_attributes[cons.TAG_SCALE] == cons.TAG_PROP_H1:
            ...
            self.toc_list.append(
                ["h1-%d" % self.toc_counters[cons.TAG_PROP_H1], text_buffer.get_text(start_iter, end_iter), node_id])
        elif self.curr_attributes[cons.TAG_SCALE] == cons.TAG_PROP_H2:
            self.toc_counters[cons.TAG_PROP_H2] += 1
            self.toc_list.append(
                ["h2-%d" % self.toc_counters[cons.TAG_PROP_H2], text_buffer.get_text(start_iter, end_iter), node_id])
        else:
            self.toc_counters[cons.TAG_PROP_H3] += 1
            self.toc_list.append(
                ["h3-%d" % self.toc_counters[cons.TAG_PROP_H3], text_buffer.get_text(start_iter, end_iter), node_id])
...
class StateMachine:
    ...
    def get_embedded_pixbufs_tables_codeboxes(self, text_buffer, for_print=0, sel_range=None):
        ...
        if sel_range:
            curr_iter = text_buffer.get_iter_at_offset(sel_range[0])
        else:
    ...
    def codebox_to_dict(self, anchor, for_print):
        ...
        if for_print == 1:
            ...
            codebox_dict['fill_text'] = anchor.sourcebuffer.get_text(*anchor.sourcebuffer.get_bounds())
        return codebox_dict
    ...
    def node_selected_changed(self, node_id):
        ...
        if self.dad.user_active and not self.dad.go_bk_fw_click:
            ...
            if self.visited_nodes_idx != last_index: del self.visited_nodes_list[
                                                         self.visited_nodes_idx + 1:last_index + 1]
    ...
    def text_variation(self, node_id, varied_text):
        ...
        if self.nodes_indicators[node_id] < 2:
            if alphanum != None:
                self.nodes_indicators[node_id] = 2  # alphanumeric transition
            else:
                self.nodes_indicators[node_id] = 1  # non alphanumeric transition

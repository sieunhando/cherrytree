...
SPECIAL_CHARS_DEFAULT = unicode("“”„‘’•◇▪▸☐☑☒★…‰€©®™°↓↑→←↔↵⇓⇑⇒⇐⇔»«▼▲►◄≤≥≠≈±¹²³½¼⅛×÷∞ø∑√∫ΔδΠπΣΦΩωαβγεηλμ☺☻☼♥♣♦✔♀♂♪♫✝",
...
def get_toolbar_ui_str(dad):
    ...
    for i, toolbar_element in enumerate(dad.toolbar_ui_list):
        if toolbar_element == cons.CHAR_STAR:
            dad.toolbar_open_n_recent = i
        elif toolbar_element == cons.TAG_SEPARATOR:
            toolbar_ui_str += "<separator/>"
        else:
            toolbar_ui_str += "<toolitem action='%s'/>" % toolbar_element
...
def config_file_load(dad):
    ...
    if os.path.isfile(cons.CONFIG_PATH):
        ...
        section = "state"
        dad.file_dir = unicode(cfg.get(section, "file_dir"), cons.STR_UTF8, cons.STR_IGNORE) if cfg.has_option(section,
                                                                                                               "file_dir") else ""
        dad.file_name = unicode(cfg.get(section, "file_name"), cons.STR_UTF8, cons.STR_IGNORE) if cfg.has_option(
            section, "file_name") else ""
        dad.toolbar_visible = cfg.getboolean(section, "toolbar_visible") if cfg.has_option(section,
                                                                                           "toolbar_visible") else True
        dad.win_is_maximized = cfg.getboolean(section, "win_is_maximized") if cfg.has_option(section,
        ...
        if cfg.has_option(section, "node_path"):
            ...
            dad.cursor_position = cfg.getint(section, "cursor_position") if cfg.has_option(section,
        ...
        dad.show_node_name_header = cfg.getboolean(section, "show_node_name_header") if cfg.has_option(section,
                                                                                                       "show_node_name_header") else True
        dad.nodes_on_node_name_header = cfg.getint(section, "nodes_on_node_name_header") if cfg.has_option(section,
                                                                                                           "nodes_on_node_name_header") else NODES_ON_NODE_NAME_HEADER_DEFAULT
        ...
        dad.expanded_collapsed_string = cfg.get(section, "expanded_collapsed_string") if cfg.has_option(section,
                                                                                                        "expanded_collapsed_string") else ""
        dad.expcollnam1 = unicode(cfg.get(section, "expcollnam1"), cons.STR_UTF8, cons.STR_IGNORE) if cfg.has_option(
            section, "expcollnam1") else ""
        ...
        dad.expcollnam2 = unicode(cfg.get(section, "expcollnam2"), cons.STR_UTF8, cons.STR_IGNORE) if cfg.has_option(
            section, "expcollnam2") else ""
        ...
        dad.expcollnam3 = unicode(cfg.get(section, "expcollnam3"), cons.STR_UTF8, cons.STR_IGNORE) if cfg.has_option(
            section, "expcollnam3") else ""
        ...
        dad.nodes_bookm_exp = cfg.getboolean(section, "nodes_bookm_exp") if cfg.has_option(section,
                                                                                           "nodes_bookm_exp") else True
        dad.nodes_icons = cfg.get(section, "nodes_icons") if cfg.has_option(section, "nodes_icons") else "c"
        dad.default_icon_text = cfg.getint(section, "default_icon_text") if cfg.has_option(section,
                                                                                           "default_icon_text") else cons.NODE_ICON_BULLET_ID
        dad.tree_right_side = cfg.getboolean(section, "tree_right_side") if cfg.has_option(section,
                                                                                           "tree_right_side") else False
        dad.cherry_wrap_width = cfg.getint(section, "cherry_wrap_width") if cfg.has_option(section,
                                                                                           "cherry_wrap_width") else 130
        dad.tree_click_focus_text = cfg.getboolean(section, "tree_click_focus_text") if cfg.has_option(section,
                                                                                                       "tree_click_focus_text") else False
        dad.tree_click_expand = cfg.getboolean(section, "tree_click_expand") if cfg.has_option(section,
                                                                                               "tree_click_expand") else False

        section = "editor"
        dad.syntax_highlighting = cfg.get(section, "syntax_highlighting") if cfg.has_option(section,
                                                                                            "syntax_highlighting") else cons.RICH_TEXT_ID
        dad.auto_syn_highl = cfg.get(section, "auto_syn_highl") if cfg.has_option(section, "auto_syn_highl") else "sh"
        dad.style_scheme = cfg.get(section, "style_scheme") if cfg.has_option(section,
                                                                              "style_scheme") else cons.STYLE_SCHEME_DARK
        dad.enable_spell_check = cfg.getboolean(section, "enable_spell_check") if cfg.has_option(section,
                                                                                                 "enable_spell_check") else False
        dad.spell_check_lang = cfg.get(section, "spell_check_lang") if cfg.has_option(section,
                                                                                      "spell_check_lang") else SPELL_CHECK_LANG_DEFAULT
        dad.show_line_numbers = cfg.getboolean(section, "show_line_numbers") if cfg.has_option(section,
                                                                                               "show_line_numbers") else False
        dad.spaces_instead_tabs = cfg.getboolean(section, "spaces_instead_tabs") if cfg.has_option(section,
        ...
        dad.embfile_show_filename = cfg.getboolean(section, "embfile_show_filename") if cfg.has_option(section,
                                                                                                       "embfile_show_filename") else True
        dad.embfile_max_size = cfg.getint(section, "embfile_max_size") if cfg.has_option(section,
                                                                                         "embfile_max_size") else MAX_SIZE_EMBFILE_MB_DEFAULT
        dad.line_wrapping = cfg.getboolean(section, "line_wrapping") if cfg.has_option(section,
                                                                                       "line_wrapping") else True
        dad.auto_smart_quotes = cfg.getboolean(section, "auto_smart_quotes") if cfg.has_option(section,
                                                                                               "auto_smart_quotes") else True
        dad.wrapping_indent = cfg.getint(section, "wrapping_indent") if cfg.has_option(section,
        ...
        dad.rt_show_white_spaces = cfg.getboolean(section, "rt_show_white_spaces") if cfg.has_option(section,
                                                                                                     "rt_show_white_spaces") else False
        dad.pt_show_white_spaces = cfg.getboolean(section, "pt_show_white_spaces") if cfg.has_option(section,
                                                                                                     "pt_show_white_spaces") else True
        dad.rt_highl_curr_line = cfg.getboolean(section, "rt_highl_curr_line") if cfg.has_option(section,
                                                                                                 "rt_highl_curr_line") else True
        dad.pt_highl_curr_line = cfg.getboolean(section, "pt_highl_curr_line") if cfg.has_option(section,
                                                                                                 "pt_highl_curr_line") else True
        dad.space_around_lines = cfg.getint(section, "space_around_lines") if cfg.has_option(section,
                                                                                             "space_around_lines") else 0
        dad.relative_wrapped_space = cfg.getint(section, "relative_wrapped_space") if cfg.has_option(section,
                                                                                                     "relative_wrapped_space") else 50
        dad.h_rule = cfg.get(section, "h_rule") if cfg.has_option(section, "h_rule") else HORIZONTAL_RULE
        dad.special_chars = unicode(cfg.get(section, "special_chars"), cons.STR_UTF8,
                                    cons.STR_IGNORE) if cfg.has_option(section,
                                                                       "special_chars") else SPECIAL_CHARS_DEFAULT
        dad.selword_chars = unicode(cfg.get(section, "selword_chars"), cons.STR_UTF8,
                                    cons.STR_IGNORE) if cfg.has_option(section,
                                                                       "selword_chars") else SELWORD_CHARS_DEFAULT
        dad.chars_listbul = unicode(cfg.get(section, "chars_listbul"), cons.STR_UTF8,
                                    cons.STR_IGNORE) if cfg.has_option(section,
                                                                       "chars_listbul") else CHARS_LISTBUL_DEFAULT
        dad.chars_toc = unicode(cfg.get(section, "chars_toc"), cons.STR_UTF8, cons.STR_IGNORE) if cfg.has_option(
            section, "chars_toc") else CHARS_TOC_DEFAULT
        ...
        dad.timestamp_format = cfg.get(section, "timestamp_format") if cfg.has_option(section,
                                                                                      "timestamp_format") else TIMESTAMP_FORMAT_DEFAULT
        dad.monospace_bg = cfg.get(section, "monospace_bg") if cfg.has_option(section,
                                                                              "monospace_bg") else DEFAULT_MONOSPACE_BG
        dad.links_underline = cfg.getboolean(section, "links_underline") if cfg.has_option(section,
                                                                                           "links_underline") else True
        dad.links_relative = cfg.getboolean(section, "links_relative") if cfg.has_option(section,
                                                                                         "links_relative") else False
        ...
        dad.codebox_width_pixels = cfg.getboolean(section, "codebox_width_pixels") if cfg.has_option(section,
                                                                                                     "codebox_width_pixels") else True
        dad.codebox_line_num = cfg.getboolean(section, "codebox_line_num") if cfg.has_option(section,
                                                                                             "codebox_line_num") else False
        dad.codebox_match_bra = cfg.getboolean(section, "codebox_match_bra") if cfg.has_option(section,
                                                                                               "codebox_match_bra") else True
        dad.codebox_syn_highl = cfg.get(section, "codebox_syn_highl") if cfg.has_option(section,
                                                                                        "codebox_syn_highl") else cons.PLAIN_TEXT_ID
        dad.codebox_auto_resize = cfg.getboolean(section, "codebox_auto_resize") if cfg.has_option(section,
                                                                                                   "codebox_auto_resize") else True
        ...
        dad.table_column_mode = cfg.get(section, "table_column_mode") if cfg.has_option(section,
        ...
        dad.rt_def_fg = cfg.get(section, "rt_def_fg") if cfg.has_option(section,
                                                                        "rt_def_fg") else cons.RICH_TEXT_DARK_FG
        dad.rt_def_bg = cfg.get(section, "rt_def_bg") if cfg.has_option(section,
                                                                        "rt_def_bg") else cons.RICH_TEXT_DARK_BG
        dad.tt_def_fg = cfg.get(section, "tt_def_fg") if cfg.has_option(section,
                                                                        "tt_def_fg") else cons.TREE_TEXT_LIGHT_FG
        dad.tt_def_bg = cfg.get(section, "tt_def_bg") if cfg.has_option(section,
                                                                        "tt_def_bg") else cons.TREE_TEXT_LIGHT_BG
        ...
        dad.col_link_webs = cfg.get(section, "col_link_webs") if cfg.has_option(section,
                                                                                "col_link_webs") else cons.COLOR_48_LINK_WEBS
        dad.col_link_node = cfg.get(section, "col_link_node") if cfg.has_option(section,
                                                                                "col_link_node") else cons.COLOR_48_LINK_NODE
        dad.col_link_file = cfg.get(section, "col_link_file") if cfg.has_option(section,
                                                                                "col_link_file") else cons.COLOR_48_LINK_FILE
        dad.col_link_fold = cfg.get(section, "col_link_fold") if cfg.has_option(section,
                                                                                "col_link_fold") else cons.COLOR_48_LINK_FOLD

        section = "misc"
        dad.toolbar_ui_list = cfg.get(section, "toolbar_ui_list").split(cons.CHAR_COMMA) if cfg.has_option(section,
                                                                                                           "toolbar_ui_list") else menus.TOOLBAR_VEC_DEFAULT
        dad.systray = cfg.getboolean(section, "systray") if cfg.has_option(section, "systray") else False
        dad.start_on_systray = cfg.getboolean(section, "start_on_systray") if cfg.has_option(section,
        ...
        dad.check_version = cfg.getboolean(section, "check_version") if cfg.has_option(section,
                                                                                       "check_version") else False
        dad.reload_doc_last = cfg.getboolean(section, "reload_doc_last") if cfg.has_option(section,
                                                                                           "reload_doc_last") else True
        dad.enable_mod_time_sentinel = cfg.getboolean(section, "mod_time_sent") if cfg.has_option(section,
        ...
        dad.autosave_on_quit = cfg.getboolean(section, "autosave_on_quit") if cfg.has_option(section,
                                                                                             "autosave_on_quit") else False
        dad.limit_undoable_steps = cfg.getint(section, "limit_undoable_steps") if cfg.has_option(section,
...
def config_file_apply(dad):
    ...
    if dad.line_wrapping:
        dad.sourceview.set_wrap_mode(gtk.WRAP_WORD)
    else:
        dad.sourceview.set_wrap_mode(gtk.WRAP_NONE)
...
def get_tree_expanded_collapsed_string(dad):
    ...
    if len(expanded_collapsed_string) > 0:
        dad.expanded_collapsed_string = expanded_collapsed_string[1:]
    else:
        dad.expanded_collapsed_string = ""
...
def set_tree_path_and_cursor_pos(dad):
    ...
    if dad.node_path:
        try:
            node_iter_to_focus = dad.treestore.get_iter(dad.node_path)
        except:
            node_iter_to_focus = None
...
def preferences_tab_text_n_code(dad, vbox_all_nodes, pref_dialog):
    ...
    def on_textbuffer_special_chars_changed(textbuffer, *args):
        new_special_chars = unicode(textbuffer.get_text(*textbuffer.get_bounds()).replace(cons.CHAR_NEWLINE, ""),
                                    cons.STR_UTF8, cons.STR_IGNORE)
    ...
    def on_spinbutton_relative_wrapped_space_value_changed(spinbutton):
        ...
        dad.sourceview.set_pixels_inside_wrap(
            get_pixels_inside_wrap(dad.space_around_lines, dad.relative_wrapped_space))
...
def preferences_tab_rich_text(dad, vbox_text_nodes, pref_dialog):
    ...
    def set_checkbutton_spell_check_model():
        ...
        combobox_spell_check_lang.set_active_iter(
            dad.get_combobox_iter_from_value(dad.spell_check_lang_liststore, 0, dad.spell_check_lang))
...
def preferences_tab_plain_text_n_code(dad, vbox_code_nodes, pref_dialog):
    ...
    combobox_style_scheme.set_active_iter(
        dad.get_combobox_iter_from_value(dad.style_scheme_liststore, 0, dad.style_scheme))
    ...
    def on_c_icon_button_clicked(button):
        ...
        if sel_key:
            ...
            c_icon_button.set_image(
                gtk.image_new_from_stock(cons.NODES_STOCKS[dad.default_icon_text], gtk.ICON_SIZE_BUTTON))
            dad.treeview_refresh(change_icon=True)
...
def preferences_tab_kb_shortcuts(dad, vbox_tool, pref_dialog):
    ...
    def edit_selected_tree_iter():
        ...
        kb_shortcut_key = kb_shortcut_full.replace(menus.KB_CONTROL, "").replace(menus.KB_SHIFT, "").replace(
            menus.KB_ALT, "")
        ...
        if response == gtk.RESPONSE_ACCEPT:
            ...
            if kb_shortcut_full:
                ...
                if in_use_name and in_use_name != action_name:
                    warning_label = "<b>" + _("The Keyboard Shortcut '%s' is already in use") % cgi.escape(
                        kb_shortcut_full) + "</b>\n\n" + _("The current associated action is '%s'") % \
                                                         dad.menudict[in_use_name]["dn"] + "\n\n<b>" + _(
                        "Do you want to steal the shortcut?") + "</b>"
                    response = support.dialog_question_warning(pref_dialog, warning_label)
                    if response == gtk.RESPONSE_ACCEPT:
...
def preferences_tab_toolbar(dad, vbox_tool, pref_dialog):
    ...
    def on_button_add_clicked(button):
        ...
        sel_key = support.dialog_choose_element_in_list(pref_dialog, _("Select Element to Add"), [], "",
                                                        icon_n_label_list)
        if sel_key:
            if sel_key == "ct_open_file": sel_key = cons.CHAR_STAR
            model, tree_iter = treeviewselection.get_selected()
            if tree_iter:
                liststore.insert_after(tree_iter, get_toolbar_entry_columns_from_key(dad, sel_key))
            else:
                liststore.append(get_toolbar_entry_columns_from_key(dad, sel_key))
...
def preferences_tab_misc(dad, vbox_misc, pref_dialog):
    ...
    combobox_country_language.set_active_iter(
        dad.get_combobox_iter_from_value(dad.country_lang_liststore, 0, dad.country_lang))
    ...
    def on_checkbutton_systray_toggled(checkbutton):
        ...
        if dad.systray:
            if not dad.use_appind:
                if "status_icon" in dir(dad.boss):
                    dad.boss.status_icon.set_property(cons.STR_VISIBLE, True)
                else:
                    dad.status_icon_enable()
            else:
                if "ind" in dir(dad.boss):
                    dad.boss.ind.set_status(appindicator.STATUS_ACTIVE)
                else:
                    dad.status_icon_enable()
        else:
            if not dad.use_appind:
                dad.boss.status_icon.set_property(cons.STR_VISIBLE, False)
            else:
                dad.boss.ind.set_status(appindicator.STATUS_PASSIVE)

    ...
    if list_info["num"] != 0:
        ...
        if list_info["num"] < 0:
            ...
            dad.replace_text_at_offset(text_to, bull_offset,
                                       bull_offset + dad.lists_handler.get_leading_chars_num(list_info["num"]),
                                       text_buffer)
    iter_start = text_buffer.get_iter_at_offset(curr_offset)
...
def on_sourceview_event_after_key_press(dad, text_view, event, syntax_highl):
    ...
    if dad.auto_smart_quotes and keyname in [cons.STR_KEY_DQUOTE, cons.STR_KEY_SQUOTE]:
                ...
                if offset_1 > 0:
                    ...
                    while iter_start:
                        ...
                        if curr_char == start_char:
                            ...
                            if not iter_start.backward_char() or iter_start.get_char() in [cons.CHAR_NEWLINE,
                                                                                           cons.CHAR_SPACE,
                                                                                           cons.CHAR_TAB]:
        ...
        if keyname == cons.STR_KEY_RETURN:
            ...
            if list_info:
                text_buffer.insert(text_buffer.get_iter_at_mark(text_buffer.get_insert()),
                                   3 * (1 + list_info["level"]) * cons.CHAR_SPACE)
        ...
        if keyname == cons.STR_KEY_RETURN:
            ...
            if list_info["num"] < 0:
                ...
                while list_info and list_info["num"] > 0:
                    ...
                    iter_start, iter_end, chars_rm = dad.lists_handler.list_check_n_remove_old_list_type_leading(
                        iter_start, iter_end, text_buffer)
                    end_offset -= chars_rm
                ...
                if iter_start.get_char() == cons.CHAR_GREATER and iter_start.backward_char():
                    if iter_start.get_line_offset() == 0:
                        ...
                        if iter_start.get_char() == cons.CHAR_LESSER:
                            # "<> " becoming "◇ "
                            dad.special_char_replace(config.CHARS_LISTBUL_DEFAULT[1], iter_start, iter_insert,
                                                     text_buffer)
                        elif iter_start.get_char() == cons.CHAR_MINUS:
                            # "-> " becoming "→ "
                            dad.special_char_replace(config.CHARS_LISTBUL_DEFAULT[4], iter_start, iter_insert,
                                                     text_buffer)
                        elif iter_start.get_char() == cons.CHAR_EQUAL:
                            # "=> " becoming "⇒ "
                            dad.special_char_replace(config.CHARS_LISTBUL_DEFAULT[5], iter_start, iter_insert,
                                                     text_buffer)
                    elif iter_start.get_char() == cons.CHAR_MINUS and iter_start.backward_char():
                        if iter_start.get_char() == cons.CHAR_LESSER:
                            # "<-> " becoming "↔ "
                            dad.special_char_replace(cons.SPECIAL_CHAR_ARROW_DOUBLE, iter_start, iter_insert,
                                                     text_buffer)
                        elif iter_start.get_char() == cons.CHAR_MINUS:
                            # "--> " becoming "→ "
                            dad.special_char_replace(cons.SPECIAL_CHAR_ARROW_RIGHT, iter_start, iter_insert,
                                                     text_buffer)
                    elif iter_start.get_char() == cons.CHAR_EQUAL and iter_start.backward_char():
                        if iter_start.get_char() == cons.CHAR_LESSER:
                            # "<=> " becoming "⇔ "
                            dad.special_char_replace(cons.SPECIAL_CHAR_ARROW_DOUBLE2, iter_start, iter_insert,
                                                     text_buffer)
                        elif iter_start.get_char() == cons.CHAR_EQUAL:
                            # "==> " becoming "⇒ "
                            dad.special_char_replace(cons.SPECIAL_CHAR_ARROW_RIGHT2, iter_start, iter_insert,
                                                     text_buffer)
                elif iter_start.get_char() == cons.CHAR_MINUS and iter_start.backward_char() \
                    ...
                    if iter_start.get_char().lower() == "c" and iter_start.backward_char() \
                        ...
                        dad.special_char_replace(cons.SPECIAL_CHAR_COPYRIGHT, iter_start, iter_insert, text_buffer)
                            ...
                            and iter_start.get_char() == cons.CHAR_PARENTH_OPEN:
                        # "(r) " becoming "® "
                        dad.special_char_replace(cons.SPECIAL_CHAR_REGISTERED_TRADEMARK, iter_start, iter_insert,
                                                 text_buffer)
                    elif iter_start.get_char().lower() == "m" and iter_start.backward_char() \
                            and iter_start.get_char() == "t" and iter_start.backward_char() \
                            and iter_start.get_char() == cons.CHAR_PARENTH_OPEN:
                        # "(tm) " becoming "™ "
                        dad.special_char_replace(cons.SPECIAL_CHAR_UNREGISTERED_TRADEMARK, iter_start, iter_insert,
                                                 text_buffer)
...
"""))
    ...
    dialog.set_artists(
        ["OCAL <http://www.openclipart.org/>", "Zeltak <zeltak@gmail.com>", "Angelo Penone <angelo.penone@gmail.com>"])
    dialog.set_translator_credits(_("Armenian") + " (hy) Seda Stamboltsyan <sedastam@yandex.com>" + cons.CHAR_NEWLINE +
                                  _(
                                      "Chinese Simplified") + " (zh_CN) Channing Wong <channing.wong@qq.com>" + cons.CHAR_NEWLINE +
                                  ...
                                  _(
                                      "Portuguese Brazil") + " (pt_BR) Vinicius Schmidt <viniciussm@rocketmail.com>" + cons.CHAR_NEWLINE +
...
def dialog_choose_element_in_list(father_win, title, elements_list, column_title, icon_n_label_list=None):
    ...
    if not icon_n_label_list:
        ...
        elements_liststore = gtk.ListStore(str, str, str)
...
def dialog_image_handle(father_win, title, original_pixbuf):
    ...
    def image_load_into_dialog():
        ...
        if img_parms.width <= 900 and img_parms.height <= 600:
            ...
            pixbuf = img_parms.original_pixbuf.scale_simple(int(img_parms.width), int(img_parms.height),
            ...
            pixbuf = img_parms.original_pixbuf.scale_simple(int(img_parms_width), int(img_parms_height),
                                                            gtk.gdk.INTERP_BILINEAR)
...
def dialog_exit_del_temp_files(dad):
    ...
    image.set_from_stock(gtk.STOCK_DIALOG_WARNING, gtk.ICON_SIZE_DIALOG)
    label = gtk.Label("<b>" + _(
        "Temporary Files were Created and Opened with External Applications.") + "</b>" + 2 * cons.CHAR_NEWLINE + "<b>" + _(
        "Quit the External Applications Before Quit CherryTree.") + "</b>" + 2 * cons.CHAR_NEWLINE + "<b>" + _(
        "Did you Quit the External Applications?") + "</b>")
    label.set_use_markup(True)
    hbox = gtk.HBox()
    hbox.pack_start(image)
    hbox.pack_start(label)
...
def dialog_exit_save(father_win):
    ...
    image = gtk.Image()
    image.set_from_stock(gtk.STOCK_DIALOG_WARNING, gtk.ICON_SIZE_DIALOG)
    label = gtk.Label("<b>" + _("The Current Document was Updated.") + "</b>" + 2 * cons.CHAR_NEWLINE + "<b>" + _(
        "Do you want to Save the Changes?") + "</b>")
    label.set_use_markup(True)
    hbox = gtk.HBox()
    hbox.pack_start(image)
    hbox.pack_start(label)
...
def dialog_link_handle(dad, title, sel_tree_iter):
    ...
    def link_type_changed_on_dialog():
        ...
        if dad.link_type == cons.LINK_TYPE_WEBS:
        ...
        elif dad.link_type == cons.LINK_TYPE_FILE:
    ...
    def on_button_browse_for_file_to_link_to_clicked(self, *args):
        ...
        if dad.links_relative:
            try:
                filepath = os.path.relpath(filepath, dad.file_dir)
            except:
                print "cannot set relative path for different drives"
    ...
    def on_browse_anchors_button_clicked(*args):
        ...
        if not anchors_list:
            ...
            return
        ret_anchor_name = dialog_choose_element_in_list(dialog, _("Choose Existing Anchor"), anchors_list,
                                                        _("Anchor Name"))
            ...
            if event.button == 2:
                ...
                if path_at_click:
                    if treeview.row_expanded(path_at_click[0]):
                        ...
                        treeview.expand_row(path_at_click[0], False)
    ...
    def on_key_press_links_handle_dialog(widget, event):
        ...
        if keyname == cons.STR_KEY_RETURN:
            ...
            if dad.link_type == cons.LINK_TYPE_WEBS:
                radiobutton_file.set_active(True)
            elif dad.link_type == cons.LINK_TYPE_FILE:
                radiobutton_folder.set_active(True)
            elif dad.link_type == cons.LINK_TYPE_FOLD:
                radiobutton_node.set_active(True)
            else:
                radiobutton_webs.set_active(True)
            return True
        return False
    ...
    if new_protection['on']:
        ...
        for bad_char in cons.CHARS_NOT_FOR_PASSWD:
            if bad_char in new_protection['p1']:
                dialog_error(_("The Characters  %s  are Not Allowed") % cons.CHAR_SPACE.join(cons.CHARS_NOT_FOR_PASSWD),
                             dad.window)
                return False
...
def dialog_selnode_selnodeandsub_alltree(dad, also_selection, also_include_node_name=False, also_new_node_page=False,
    ...
    content_area.show_all()
    response = dialog.run()
    if radiobutton_selnode.get_active():
        ret_val = 1
    elif radiobutton_selnodeandsub.get_active():
        ret_val = 2
    elif radiobutton_alltree.get_active():
        ret_val = 3
    else:
        ret_val = 4
    if also_include_node_name:
...
def set_menu_items_recent_documents(dad):
    ...
    for target in [1, 2]:
        for i, filepath in enumerate(dad.recent_docs):
            ...
            menu_item.show()
            if target == 1:
                dad.recent_menu_1.append(menu_item)
            else:
                dad.recent_menu_2.append(menu_item)
    if first_run:
        # main menu
